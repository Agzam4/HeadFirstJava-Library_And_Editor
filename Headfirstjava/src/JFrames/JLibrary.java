...
public class JLibrary extends JFrame {
	...
	public JLibrary() {
		...
		JButton find = new JButton("Õ‡ÈÚË");
...
//		        tb.width = 7; TODO
		...
		JLabel lblNewLabel_1 = new JLabel("»Ïˇ ÍÎ‡ÒÒ‡: ");
		...
		JLabel lblNewLabel = new JLabel("—Ú‡ÌËˆ‡: ");
	...
	private void reset—haptersLenght() {
		for (int i = 0; i < chaptersLenght.length; i++) {
			chaptersLenght[i] = 0;
		}
	}
	...
	private void AddTiles(JPanel sp, String chap, String page, String name) {
		...
		for (int i = 0; i < ids.size(); i++) {
			if(chaps != uploader.chapter.get(ids.get(i))) {
				...
				JLabel bg = new JLabel("√Î‡‚‡ " + (chaps+1));
	...
	private void setMouseListener(JComponent c, JPanel pp, int id) {
		c.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				JCode.main(uploader.code.get(id), "√Î‡‚‡ " + (uploader.chapter.get(id)+1) +
						" - ÒÚ " + uploader.page.get(id) + " | " + uploader.name.get(id), id, uploader);
			}
		});
	}
}
